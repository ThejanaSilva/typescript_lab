...
export function BingoModal({ mode, onDismiss }: BingoModalProps) {
  const title = mode === 'bingo' ? 'B I N G O !' : 'H U N T   C O M P L E T E';
  const subtitle =
    mode === 'bingo'
      ? '≫ You completed a line! ≪'
      : '≫ You marked every mission target! ≪';
  ...
  return (
    <div className="cyber-modal-overlay fixed inset-0 flex items-center justify-center p-4 z-50">
      <div className="cyber-modal rounded-xl p-8 max-w-sm w-full text-center animate-bounce" style={{animationDuration: '0.6s'}}>
        ...
        <p className="cyber-muted mb-8 text-lg font-bold tracking-wider uppercase">{subtitle}</p>
